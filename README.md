...
为了提高代码的可维护性和可读性，`image_show.py`已被重构为一个协调器模式，将功能分离到以下独立模块：

- **image_show.py**：轻量级协调器，负责初始化和整合各模块
- **config_manager.py**：配置文件加载和管理
- **data_processor.py**：CSV数据加载、过滤和处理
- **ui/chart.py**：图表渲染和交互控制

这种模块化设计使代码职责更明确，便于维护和测试。
