...
# 加载测试数据
test_data_path = os.path.join(os.path.dirname(__file__), "../../data/test_config.json")
...
reported_by_mapping = {int(index): reported_by for index, reported_by in reported_by_mapping.items()}
...
def test_reported_by_matching():
    ...
    for scenario in test_scenarios:
        ...
        main_window.comboBox_TesterLocation.setCurrentIndex(scenario["tester_location_index"])
...
# 将映射关系转换为参数化测试所需的格式
parametrize_data = [(index, reported_by) for index, reported_by in reported_by_mapping.items()]
