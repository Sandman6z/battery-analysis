...
def run_pylint():
    ...
    if not scripts_dir.exists():
        logger.error(f"错误：脚本目录 {scripts_dir} 不存在！")
        sys.exit(1)
    if not tests_dir.exists():
        logger.error(f"错误：测试目录 {tests_dir} 不存在！")
        sys.exit(1)

    logger.info(f"正在分析目录：{src_dir}, {scripts_dir}, {tests_dir}")
    ...
    try:
        ...
        # 生成HTML报告
        generate_html_report(project_root, src_dir, scripts_dir, tests_dir)
...
def generate_html_report(project_root, src_dir, scripts_dir, tests_dir):
    """生成HTML格式的Pylint报告"""
