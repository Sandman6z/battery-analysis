...
class ExcelReportWriter:
    ...
    def _write_result_columns(self, wsResult, fmts):
        ...
        for c in range(self.intCurrentLevelNum):
            wsResult.write(
                ...
                f"{self.listCurrentLevel[c]}mA",
                fmts["result_data"],
            )
            wsResult.merge_range(
                1,
                2 + c * (2 + self.intVoltageLevelNum),
                1,
                (c + 1) * (2 + self.intVoltageLevelNum) - 1,
                "Voltage",
                fmts["result_data"],
            )
    ...
    def _write_overview_statistics(
        self, wsOverview, stats, fmts, wsOverviewStatisticalStartLine=13
    ):
        """写入Overview工作表中的统计结果"""
        wsOverview.write(
            wsOverviewStatisticalStartLine, 0, "Statisticals Results", fmts["overview_stat_dark"]
        )
        ...
        for v in range(1, self.intVoltageLevelNum + 1):
            ...
            wsOverview.write_rich_string(
                wsOverviewStatisticalStartLine,
                v,
        ...
        for c in range(1, self.intCurrentLevelNum + 1):
            ...
            wsOverview.write_rich_string(
                wsOverviewStatisticalStartLine + c,
                0,
                fmts["overview_stat_light"],
                "Pulse Current ",
                fmts["overview_stat_light_bold"],
    ...
    def _write_sample_excel(self, wsExcel, sample, stats, fmts):
        ...
        if intTestProfileStartLine == 4:
            wsExcel.merge_range(
                0, intTestProfileStartLine, 1, intTestProfileStartLine, "", fmts["sample_line"]
            )
        wsExcel.write(0, intTestProfileStartLine, listStrItems[4], fmts["sample_line"])
        for i in range(5, 11):
            wsExcel.merge_range(
                0,
                intTestProfileStartLine + (i - 4),
                1,
                intTestProfileStartLine + (i - 4),
                listStrItems[i],
                fmts["sample_line"],
        ...
        wsExcel.merge_range(
            0,
            intTestProfileStartLine + 7,
            1,
            intTestProfileStartLine + 8,
            listStrItems[11],
            fmts["sample_line"],
        )
        wsExcel.merge_range(
            0,
            intTestProfileStartLine + 9,
            0,
            intTestDateStartCol - 1,
            listStrItems[13],
            fmts["sample_line"],
        )
        for v in range(self.intVoltageLevelNum):
            wsExcel.merge_range(
                1,
                intTestProfileStartLine + 9 + v * 2,
                1,
                intTestProfileStartLine + 9 + v * 2 + 1,
                f"{self.listVoltageLevel[v]}V",
                fmts["sample_line"],
            )
        ...
        if len(listStrContent[4].split("\\")) == 1:
            wsExcel.write(2, intTestProfileStartLine, listStrContent[4], fmts["sample_data"])
        else:
            url_path = listStrContent[4].replace("\\", "/")
            wsExcel.write_url(
                2,
                intTestProfileStartLine,
                f"file:///{url_path}",
                fmts["hyperlink"],
                string=listStrContent[4].split("\\")[-1],
            )
        for i in range(5, 13):
            wsExcel.write(
                2, intTestProfileStartLine + (i - 4), listStrContent[i], fmts["sample_data"]
            )
        ...
        for i in range(4):
            ...
            wsExcel.write(2, intTestDateStartCol + i, listStrContent[14 + i], fmt)

        wsExcel.write(2, intTestDateStartCol + 5, listStrContent[19], fmts["sample_data"])

        # 测试结果超链接
        url_path = listStrContent[18].replace("\\", "/")
        wsExcel.write_url(
            2,
            intTestDateStartCol + 4,
            f"file:///{url_path}",
            fmts["hyperlink"],
            string=listStrContent[18].split("\\")[-1],
        )
    ...
    def _write_sample_word(self, wsWord, sample, stats, fmts):
        ...
        for i in range(4, 12):
            wsWord.write(intTestProfileStartLine + (i - 4), 0, listStrItems[i], fmts["word_line"])
        wsWord.merge_range(
            intTestProfileStartLine + 8,
            0,
            intTestProfileStartLine + 10,
            0,
            listStrItems[13],
            fmts["word_line"],
        )
        for i in range(6):
            wsWord.write(intTestDateStartRow + i, 0, listStrItems[14 + i], fmts["word_line"])
        ...
        if intTestProfileStartLine == 4:
            wsWord.merge_range(
                ...
                1,
                intTestProfileStartLine,
                intActualMeasuredCapacityLength,
                "",
                fmts["word_data"],
            )
        if len(listStrContent[4].split("\\")) == 1:
            wsWord.write(intTestProfileStartLine, 1, listStrContent[4], fmts["word_data"])
        else:
            url_path = listStrContent[4].replace("\\", "/")
            wsWord.write_url(
                intTestProfileStartLine,
        ...
        wsWord.merge_range(
            ...
            1,
            intTestProfileStartLine + 7,
            int(intActualMeasuredCapacityLength / 2),
            listStrContent[11],
            fmts["word_data"],
        )
        wsWord.merge_range(
            intTestProfileStartLine + 7,
            int(intActualMeasuredCapacityLength / 2) + 1,
            intTestProfileStartLine + 7,
            intActualMeasuredCapacityLength,
            listStrContent[12],
            fmts["word_data"],
        ...
        wsWord.merge_range(
            intTestDateStartRow + 3,
            1,
            intTestDateStartRow + 3,
            intActualMeasuredCapacityLength,
            listStrContent[17],
            fmts["word_data_yellow"],
        )
        url_path = listStrContent[18].replace("\\", "/")
        display_text = listStrContent[18].split("\\")[-1]
        wsWord.merge_range(
            intTestDateStartRow + 4,
            1,
            intTestDateStartRow + 4,
            intActualMeasuredCapacityLength,
            display_text,
    ...
    def write(self, list_cpt=None, stats=None) -> None:
        ...
        if stats is None:
            stats = _compute_statistics(list_cpt, self.intCurrentLevelNum, self.intVoltageLevelNum)
