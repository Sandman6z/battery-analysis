...
class FileValidator:
    ...
    def validate_file_extension(self, filename: str, expected_extensions: list) -> Tuple[bool, str]:
        ...
        if file_ext not in expected_extensions:
            return False, f"Invalid file format: {filename} is not a valid {', '.join(expected_extensions)} file"
        return True, ""
    ...
    def validate_excel_filename(self, filename: str) -> Tuple[bool, str]:
        ...
        if 'DC' not in base_name:
            return False, f"Invalid filename format: {filename} missing batch date code (starting with DC)"
