...
def filter_data(
    plt_charge_list: list, plt_voltage_list: list, times=5, slope_max=0.2, difference_max=0.05
):
    """
        ...
        for charge_data, voltage_data in zip(plt_charge_list, plt_voltage_list):
            try:
                ...
                with np.errstate(divide="ignore", invalid="ignore"):
                    ...
                    slopes = np.abs(
                        np.where(charge_diff != 0, voltage_diff / charge_diff, slope_max)
                    )
                ...
                # 处理numpy转换错误
                logging.warning(
                    f"Error using numpy to filter data, switching to fallback algorithm: {e}"
                )
                # 切换到备用算法
                ...
                for i in range(1, len(charge_data)):
                    try:
                        ...
                        logging.warning(
                            f"Encountered non-numeric value while filtering data, skipping this point: {inner_e}"
                        )
                        continue
        ...
        for charge_data, voltage_data in zip(plt_charge_list, plt_voltage_list):
            ...
            if charge_data and voltage_data:
                try:
                    ...
                    logging.warning(
                        f"Encountered non-numeric value while filtering data, skipping this battery data: {e}"
                    )
                    continue
                ...
                for i in range(1, len(charge_data)):
                    try:
                        ...
                        logging.warning(
                            f"Encountered non-numeric value while filtering data, skipping this point: {e}"
                        )
                        continue
...
def detect_outliers(
    data: list, data_name: str, result_index: int, test_results: list, precomputed_stats=None
) -> list:
    """
    检测数据中的异常值，使用3σ原则
