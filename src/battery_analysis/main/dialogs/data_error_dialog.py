...
class DataErrorRecoveryDialog:
    ...
    def show(self, error_msg: str):
        ...
        if dialog.exec() == QW.QDialog.DialogCode.Accepted:
            ...
            if selected_id == 1:
                # 重新选择数据目录
                self.main_window.statusBar_BatteryAnalysis.showMessage("Opening data directory selector...")
                self._open_data_directory_dialog()

            elif selected_id == 2:
                # 使用默认配置重新启动
                self.main_window.statusBar_BatteryAnalysis.showMessage("Restarting with default configuration...")
    ...
    def _open_data_directory_dialog(self):
        ...
        try:
            ...
            if directory:
                self.main_window.statusBar_BatteryAnalysis.showMessage(f"Selected directory: {directory}")
