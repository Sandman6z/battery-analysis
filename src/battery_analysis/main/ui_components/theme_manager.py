    ...
    def set_theme(self, theme_name) -> None:
        ...
        try:
            if theme_name == "System Default":
                # 使用系统默认样式
                app.setStyle(
                    QW.QStyleFactory.create("windowsvista" if sys.platform == "win32" else "fusion")
                )
                self.main_window.statusBar_BatteryAnalysis.showMessage(
                    _("Switched to System Default theme")
                )
...
"""
                    ...
                    self.main_window.statusBar_BatteryAnalysis.showMessage(
                        _("Switched to Simple Dark theme")
                    )
        except (ImportError, AttributeError, TypeError, RuntimeError) as e:
            self.logger.error("Failed to switch theme: %s", e)
            self.main_window.statusBar_BatteryAnalysis.showMessage(f"Failed to switch theme: {e!s}")
