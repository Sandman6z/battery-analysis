...
class ApplicationInitializer:
    ...
    def _setup_global_exception_handler(self):
        ...
        def handle_exception(exctype, value, traceback):
            ...
            # 记录异常信息
            logger.critical(
                "System crash - uncaught exception:", exc_info=(exctype, value, traceback)
            )
            ...
            try:
                ...
                print(f"Application error: {value!s}")
    ...
    def _setup_qt_message_handler(self):
        ...
        def qt_message_handler(mode, context, message):
            ...
            if mode == QApplication.qtHandlerType.CriticalMsg:
                logger.critical(
                    "Qt critical error: %s (file: %s, line: %d)",
                    message,
                    context.file,
                    context.line,
                )
                ...
                logger.warning(
                    "Qt warning: %s (file: %s, line: %d)", message, context.file, context.line
                )
            elif mode == QApplication.qtHandlerType.InfoMsg:
                logger.debug(
                    "Qt info: %s (file: %s, line: %d)", message, context.file, context.line
                )
            else:
                logger.debug(
                    "Qt debug: %s (file: %s, line: %d)", message, context.file, context.line
                )
    ...
    def _show_startup_error_dialog(self, e):
        ...
        try:
            ...
            print(f"Application failed to start: {e!s}")
            print("Details have been logged to the log file.")
