...
class EnvironmentInitializationStep(InitializationStep):
    ...
    def execute(self, main_window) -> bool:
        ...
        try:
            ...
            # 使用环境适配器初始化环境检测器
            main_window.env_detector = (
                main_window.environment_adapter.initialize_environment_detector()
            )

            # 从环境检测器获取环境信息
            if hasattr(main_window, "env_detector") and main_window.env_detector:
                env_info = main_window.env_detector.get_environment_info()
                main_window.env_info.clear()
                main_window.env_info.update(env_info)
            elif main_window.environment_adapter.env_service:
                # 从环境服务获取环境信息
                env_info = main_window.environment_adapter.env_service.get_environment_info()
                main_window.env_info.clear()
                main_window.env_info.update(env_info)
