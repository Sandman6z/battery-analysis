...
def main():
    ...
    # 提取所有问题类型
    issue_types = re.findall(r'## 问题类型: (\w[-\w]+)', content)
    
    # 生成目录
    toc = "## 目录\n"
    for i, issue_type in enumerate(issue_types, 1):
        description = issue_descriptions.get(issue_type, "")
        toc += f"{i}. [{issue_type}](#问题类型-{issue_type})  # {description}\n"
    toc += "\n"
    
    # 在概述部分后插入目录
    new_content = re.sub(r'(## 概述\n.*?\n\n)(## 问题类型:)', r'\1' + toc + '\2', content, flags=re.DOTALL)
    
    # 写回文件
    with open('refactoring.md', 'w', encoding='utf-8') as f:
        f.write(new_content)
    
    print("已成功更新refactoring.md文件，添加了目录和中文注释")
